async function overdriveTransformer(audioBuffer, opts) {

  opts.preBand = opts.preBand === undefined ? 0.5 : opts.preBand;
  opts.color = opts.color === undefined ? 8000 : opts.color;
  opts.drive = opts.drive === undefined ? 0.5 : opts.drive;
  opts.postCut = opts.postCut === undefined ? 3000 : opts.postCut;
  ...
  let overdrive = new __Overdrive(ctx, {
    preBand: opts.preBand,
    color: opts.color,
    drive: opts.drive,
    postCut: opts.postCut
  });

  source.connect(overdrive.input);
  overdrive.connect(ctx.destination);
...
function __Overdrive (context, opts) {
...
__Overdrive.prototype = Object.create(null, {
